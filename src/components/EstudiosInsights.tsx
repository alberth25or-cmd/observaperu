...
export default function EstudiosInsights({ kpis }: EstudiosInsightsProps) {
  ...
  const topTipo = Object.entries(kpis.pctPorTipo).sort((a, b) => b[1] - a[1])[0];
  const topArea = Object.entries(kpis.pctPorArea).sort((a, b) => b[1] - a[1])[0];
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-4 sm:p-5">
      ...
      <ul className="space-y-2 text-xs sm:text-sm text-slate-700">
        <li>
          <strong>Estudios concluidos:</strong>{" "}
          {kpis.pctConcluidos}% de los candidatos declaran al menos un estudio universitario concluido.
        </li>
        <li>
          <strong>Concentración:</strong> La concentración en pocas universidades es {conc} (índice HHI: {kpis.hhi}).
          {kpis.top10Universidades[0] && (
            <> La universidad con más candidatos es {kpis.top10Universidades[0].nombre}, con {kpis.top10Universidades[0].count} candidatos.</>
          )}
        </li>
        <li>
