...
function extractLugar(biografia) {
  ...
  for (const pattern of patterns) {
    ...
    if (match && match[1]) {
      ...
      const lugarLimpio = palabras
        .filter((p) => !palabrasInvalidas.includes(p.toLowerCase()))
        .join(" ");

      if (
        !/\b\d{4}\b/.test(lugarLimpio) &&
        lugarLimpio.length > 2 &&
        lugarLimpio.length < 100
      ) {
        // Capitalizar
        return lugarLimpio
          .split(" ")
  ...
  if (idx !== -1) {
    ...
    const matchEn = desdeNacimiento.match(
      /en\s+(?:la\s+)?(?:ciudad\s+de\s+|distrito\s+de\s+|provincia\s+de\s+)?([^.,]+?)(?:\s+el\s+[^.,]+?)?(?:\.|,|$)/i,
    );
...
// CSV
const csv =
  "slug,nombre,lugar_nacimiento,fuente\n" +
  resultados
    .map(
      (r) => `"${r.slug}","${r.nombre}","${r.lugar_nacimiento}","${r.fuente}"`,
    )
    .join("\n");
fs.writeFileSync(
  path.join(outputDir, "candidatos_lugares_nacimiento.csv"),
  csv,
  "utf-8",
);
...
const encontrados = resultados.filter(
  (r) => r.lugar_nacimiento !== "No encontrado",
).length;
