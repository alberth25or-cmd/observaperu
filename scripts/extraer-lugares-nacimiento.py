...
def extract_from_biografia(biografia: str) -> Optional[str]:
    ...
    pattern3 = (
        r"naci[óo]da?\s+en\s+([^,]+?),\s*(?:departamento\s+de\s+)?([^.,]+?)(?:\.|$)"
    )
...
def extract_from_detail_file() -> dict[str, dict[str, Optional[str]]]:
    ...
    resultados: dict[str, dict[str, Optional[str]]] = {}
...
def main():
    ...
    print(
        f"  [OK] Encontrados en biografias: {encontrados_biografia}/{len(CANDIDATOS_SLUGS)}"
    )
    print()
    ...
    datos_finales: list[dict[str, str]] = []
    ...
    for slug in CANDIDATOS_SLUGS:
        ...
        fuente_final = (
            datos["fuente_biografia"] or datos.get("fuente_pdf") or "no_encontrado"
        )
    ...
    encontrados_total = sum(
        1 for d in datos_finales if d["lugar_nacimiento"] != "No encontrado"
    )
