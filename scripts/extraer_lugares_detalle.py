    ...
    for campo, campo_patterns in patterns.items():
        ...
        for pattern in campo_patterns:
            ...
            if match:
                ...
                valor = re.sub(r'\s*(DEPARTAMENTO|PROVINCIA|DISTRITO|PA[ÍI]S)[:\s]*', '', valor, flags=re.IGNORECASE)
        ...
        palabras_invalidas = [
            'LUGAR DE DOMICILIO', 'LUGAR DE NACIMIENTO', 'Lugar de domicilio', 'Lugar de nacimiento',
            'DEPARTAMENTO', 'PROVINCIA', 'DISTRITO', 'PAÍS', 'PAIS',
            'ÚNICO NACIONAL', 'UNICO NACIONAL'
        ]
...
def main():
    ...
    nacimiento_completo = sum(1 for r in resultados 
                              if r["nacimiento_pais"] != "No se encontró" 
                              and r["nacimiento_departamento"] != "No se encontró")
    domicilio_completo = sum(1 for r in resultados 
    ...
    for r in resultados[:3]:
        if r["nacimiento_departamento"] != "No se encontró":
            print(f"  {r['slug']}:")
            print(f"    Nacimiento: {r['nacimiento_distrito']}, {r['nacimiento_provincia']}, {r['nacimiento_departamento']}")
            print(f"    Domicilio: {r['domicilio_distrito']}, {r['domicilio_provincia']}, {r['domicilio_departamento']}")
