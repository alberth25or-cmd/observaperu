...
function extractFromBiografia(biografia) {
  ...
  let match = textLower.match(
    /naci[óo]da?\s+el\s+[^,]+?\s+en\s+(?:la\s+)?(?:ciudad\s+de\s+|distrito\s+de\s+|provincia\s+de\s+)?([^.,]+?)(?:\.|,|$)/,
  );
  if (match && match[1]) {
  ...
  match = textLower.match(
    /naci[óo]da?\s+en\s+(?:la\s+)?(?:ciudad\s+de\s+|distrito\s+de\s+|provincia\s+de\s+)?([^.,]+?)(?:\.|,|$)/,
  );
  if (match && match[1]) {
    let lugar = match[1].trim();
    if (!/\b\d{4}\b/.test(lugar) && lugar.length > 2) {
  ...
  match = textLower.match(
    /naci[óo]da?\s+en\s+([^,]+?),\s*(?:departamento\s+de\s+)?([^.,]+?)(?:\.|$)/,
  );
  ...
  match = textLower.match(
    /naci[óo]da?\s+el\s+[^,]+?\s+en\s+el\s+distrito\s+de\s+([^,]+?)(?:,\s+provincia\s+de\s+([^,]+?))?(?:,\s+departamento\s+de\s+([^.,]+?))?(?:\.|$)/,
  );
  if (match && match[1]) {
...
const csvRows = resultados
  .map((r) => `"${r.slug}","${r.nombre}","${r.lugar_nacimiento}","${r.fuente}"`)
  .join("\n");
