...
def count_propuestas(text: str) -> int:
    ...
    numbered_lists = re.findall(
        r"(?:^|\s)(?:[0-9]+[\.\)])\s+[A-ZÁÉÍÓÚÑ]", text, re.MULTILINE
    )
    count += len(numbered_lists)

    # 2. Contar viñetas (•, -, *, o) al inicio de línea
    bullets = re.findall(
        r"^(?:[\u2022\u2023\u25E6\u2043\-\*o])\s+[A-ZÁÉÍÓÚÑ]", text, re.MULTILINE
    )
    count += len(bullets)
    ...
    lettered_lists = re.findall(
        r"(?:^|\s)(?:[a-z]|[ivxlcdm]+)\)\s+[A-ZÁÉÍÓÚÑ]", text_lower, re.MULTILINE
    )
...
def count_metas_cuantificadas(text: str) -> int:
    ...
    plazos = re.findall(
        r"(?:para|hasta|en|al)\s+(?:el\s+)?(?:año\s+)?(\d{4})", text_lower
    )
...
def count_actividades(text: str) -> int:
    ...
    cronogramas = re.findall(
        r"(?:cronograma|plan\s+de\s+ejecución|calendario|agenda)[:\s]+", text_lower
    )
...
def extract_features(slug: str) -> dict[str, str | int]:
    """
    ...
    features: dict[str, str | int] = {
        ...
        "metas_cuantificadas_count": (
            count_metas_cuantificadas(texto_plan_completo) if texto_plan else 0
        ),
        "anios_experiencia": extract_anios_experiencia(texto_hoja_vida),
        "actividades_count": (
            count_actividades(texto_plan_completo) if texto_plan else 0
        ),
...
def compute_scores(features: dict[str, str | int]) -> dict[str, str | float]:
    """
    ...
    propuestas_count = int(features["propuestas_count"])
    propuestas_score = normalize_truncated(propuestas_count, MAX_PROPUESTAS)

    # Score de experiencia (normalización truncada)
    experiencia_count = int(features["anios_experiencia"])
    experiencia_score = normalize_truncated(experiencia_count, MAX_EXPERIENCIA)

    # Score de gestión (actividades + propuestas combinadas)
    actividades_count = int(features["actividades_count"])
    gestion_raw = actividades_count + (propuestas_count * 0.5)
    gestion_score = normalize_truncated(gestion_raw, MAX_GESTION)

    # Score de formación (ordinal)
    formacion_score = FORMACION_SCORES.get(str(features["grado_max"]), 30)

    # Score de impacto social (metas cuantificadas)
    impacto_social_count = int(features["metas_cuantificadas_count"])
    impacto_social_score = normalize_truncated(impacto_social_count, MAX_IMPACTO_SOCIAL)

    return {
        "slug": str(features["slug"]),
...
def process_candidate(
    slug: str,
) -> tuple[str, dict[str, str | int] | None, dict[str, str | float] | None]:
    """
...
def process_candidates_parallel(
    slugs: list[str],
) -> tuple[list[dict[str, str | int]], list[dict[str, str | float]]]:
    """
    ...
    """
    all_features: list[dict[str, str | int]] = []
    all_scores: list[dict[str, str | float]] = []

    # Diccionario para mantener orden original
    results_dict: dict[str, tuple[dict[str, str | int], dict[str, str | float]]] = {}

    # Procesar en paralelo
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # Enviar todas las tareas
        future_to_slug = {
            executor.submit(process_candidate, slug): slug for slug in slugs
        }
        ...
        for future in as_completed(future_to_slug):
            ...
            try:
                _, features, scores = future.result()
...
def save_outputs(
    all_features: list[dict[str, str | int]], all_scores: list[dict[str, str | float]]
) -> None:
    """
...
def main():
    ...
    if all_scores:
        ...
        print(
            df[
                ["propuestas", "experiencia", "gestion", "formacion", "impacto_social"]
            ].describe()
        )
